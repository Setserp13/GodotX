...
static func denormalize_point_component(rect, value, axis): return rect.position[axis] + rect.size[axis] * value
...
static func axis_contains_point(rect, point, axis=0):
	return rect.position[axis] <= point and point <= rect.end[axis]
...
static func transform_rect(rect, transform):
	var position = transform * rect.position
	var end = transform * rect.end
...
static func mid_edges(rect):
	return [Vector2(0,0.5), Vector2(1,0.5), Vector2(0.5,0), Vector2(0.5,1)].map(func(x):
		return denormalize_point(rect, x))
